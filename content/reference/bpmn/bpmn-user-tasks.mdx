...
Within a User Task it is possible to retrieve and set variables of the process instance and within the global Data Store. (For a better description of the existing types of Variables/Data Objects, have a look into [the `variable.XYZ()` library description for Script Tasks](../script-task-api#variable) )
This is accomplished by modifying the HTML Code of the User Task.

As an example for explaining the writing and retrieving of variables, we use the following BPMN-process where the UserTasks will interact with the variables by first entering a user's full name. 
The set variables can then be retrieved to be shown in the next User Task.

![BPMN Process](/images/bpmn/BPMN_User_Task.png)

### Set variables

User input can be used to set variables. 
Setting process variables is done like in a normal HTML forms, so the UserTask can use every usual form element. 

Typically, this is done with `<input type="..." name="variableName" value="initialValue" ...>` where `name` is the name of the process variable and value the inserted/changed/default value in the input field.
